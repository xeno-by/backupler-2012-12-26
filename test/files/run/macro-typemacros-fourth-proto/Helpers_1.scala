...
object File {
  def copy(from: String, to: String): Unit = {
    ...
    try {
      val bucket = new scala.Array[Byte](32*1024)
      inStream = new BufferedInputStream(new FileInputStream(new File(from)))
      outStream = new BufferedOutputStream(new FileOutputStream(new File(to), false))
      var bytesRead = 0
      while (bytesRead != -1) {
        bytesRead = inStream.read(bucket)
        if (bytesRead > 0) outStream.write(bucket, 0, bytesRead);
      }
    }
    finally {
