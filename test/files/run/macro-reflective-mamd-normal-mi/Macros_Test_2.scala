...
object Test extends App {
  ...
  val macrobody = Apply(Select(Ident(definitions.PredefModule), newTermName("macro")), List(Select(Ident(newTermName("Impls")), newTermName("foo"))))
  val macroparam = ValDef(NoMods, newTermName("x"), TypeTree(definitions.IntClass.asType), EmptyTree)
  val macrodef = DefDef(NoMods, newTermName("foo"), Nil, List(List(macroparam)), TypeTree(), macrobody)
