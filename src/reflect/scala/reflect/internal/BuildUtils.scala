...
trait BuildUtils extends base.BuildUtils { self: SymbolTable =>
  ...
  class BuildImpl extends BuildBase {
    ...
    def newFreeType(name: String, value: => Any = null, flags: Long = 0L, origin: String = null): FreeTypeSymbol =
      newFreeTypeSymbol(newTypeName(name), value, (if (flags == 0L) PARAM else flags) | DEFERRED, origin)

    def newFreeExistential(name: String, value: => Any = null, flags: Long = 0L, origin: String = null): FreeTypeSymbol =
      newFreeTypeSymbol(newTypeName(name), value, (if (flags == 0L) EXISTENTIAL else flags) | DEFERRED, origin)
