...
*/
   ...
   *  If the tree is not an application, it will be returned as-is with targs and vargss set to Nil.
   */
  object Application {
    def unapply(tree: Tree): Option[(Tree, List[Tree], List[List[Tree]])] = {
      ...
      if (tree == unwrapped) Some((tree, Nil, Nil))
        ...
        val vargss = valueArgumentss(tree)
        Some((unwrapped, targs, vargss))
      }
    }
  }
  ...
  def valueArgumentss(tree: Tree): List[List[Tree]] = tree match {
    case Apply(fn, args) => valueArgumentss(fn) :+ args
    case _ => Nil
  }
