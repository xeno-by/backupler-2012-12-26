...
//      val defParamss = macroDef.paramss
  ...
  /** The classloader that is used to resolve and run macro implementations.
   *  Loads classes from -cp (aka library classpath in scalac parlance).
   *  Is capable of detecting REPL and reusing its classloader if necessary.
   */
  private lazy val macroClassLoader: ClassLoader = {
    if (global.forMSIL)
      throw new UnsupportedOperationException("Scala reflection not available on this platform")

    macroLogVerbose("macro classloader: initializing from -cp: %s".format(global.classPath.asURLs))
