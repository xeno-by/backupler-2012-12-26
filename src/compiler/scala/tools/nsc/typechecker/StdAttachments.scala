...
trait StdAttachments {
  ...
  /** Scratchpad for the macro expander, which is used to store all intermediate dara except the details about the runtime.
   */
  case class MacroExpanderAttachment(original: Tree, desugared: Tree, role: MacroRole, enclosingTemplate: Tree)
 ...
 def macroExpanderAttachment(tree: Tree): MacroExpanderAttachment =
    tree.attachments.get[MacroExpanderAttachment] getOrElse {
      tree match {
        ...
        case _ => MacroExpanderAttachment(tree, EmptyTree, TERM_ROLE, EmptyTree)
      }
    }
   ...
   */
  def linkExpandeeAndDesugared(expandee: Tree, desugared: Tree, role: MacroRole, template: Tree): Unit = {
